...
<div class="max-w-[700px] mx-auto mt-[100px]">
    <div class="flex gap-[7px] items-center">
        <span class="text-black font-medium text-[24px]">{{ $getRecord()->manufacturer->name }}</span>
        <span class="text-black font-medium text-[24px]">{{ $getRecord()->model->name  }}</span>
        <span class="ml-12px text-gray-500 text-[24px]">{{ $getRecord()->manufacture_year }}</span>
    </div>
    ...
    <div class="h-[517px] mt-[30px]">
        <img src="{{$getRecord()->getMedia('*')[0]->getUrl()}}" alt="{{$getRecord()->manufacturer->name . ' ' . $getRecord()->model->name}}" class="w-full h-full rounded-3xl object-cover">
    </div>
    <div class="min-h-[200px] mt-[40px] bg-white rounded-2xl cursor-pointer">
        ...
        <p class="pt-4 pl-4 text-[14px] text-gray-500">{{ $getRecord()->description }}</p>
    </div>
